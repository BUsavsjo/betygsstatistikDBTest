async function loadAll(){
  ...
  setStatus('loading', 'Hämtar tillgängliga datakällor...', 'Läser in underlag från Skolverket.');
  ...
  try{
    setStatus('loading', 'Söker lokal SCB-import...', `Letar efter inläst lokal data för ${year.label}.`);
    ...
    if(STATIC_PAGES_BUILD || IS_GITHUB_PAGES){
      const message = 'Ingen publicerad data hittades för valt läsår.';
      log('GitHub Pages-läge: teknisk proxy är inte tillgänglig', {
      ...
      $('tableRows').innerHTML = '<tr><td colspan="3" class="muted">Tekniska datakällor kan bara upptäckas via lokal server/proxy.</td></tr>';
      renderMetricRows();
      return;
    }
    const discovered = [];
    ...
    setStatus('loading', 'Läser tekniskt underlag...', 'Väntar kort mellan anrop för att undvika för många förfrågningar.');
    ...
    renderTables();
    setStatus('loading', 'Hämtar vyer och jämförelsetal...', 'Matchar underlag och testar hämtning.');
    ...
    setStatus(ok ? 'ok' : 'warn', ok ? `Data hämtad: ${ok} vyer fungerar.` : 'Datakällan svarade, men inga önskade vyer hittades.', 'Se Vilken data som finns och Teknisk diagnostik för detaljer.');
  }catch(e){
