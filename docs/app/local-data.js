async function tryLoadLocalSource(base, sourceKind){
  try{
    ...
    return {base, sourceKind, isDemo: sourceKind === 'demo' || norm(manifest.source).includes('dummy') || norm(manifest.source).includes('demo'), manifest, overview, svSva, distribution, npPass, npRelation};
  }catch{
    return null;
  }
}
...
function schoolLabel(row){
  ...
  return row.skolenhetsnamn || row.skolenhetskod || 'OkÃ¤nd skolenhet';
}
...
function populateLocalFilters(local){
  ...
  populateSelect('gradeFilter', grades.map(g => ({value:g, label:`Ã…k ${g}`})), state.filters.grades);
...
function renderLocalOutcomes(local, meritRows){
  ...
  makeChart('knowledgeChart','bar',{
    labels: knowledgeChartRows.map(r => `Ã…k ${r.arskurs} ${schoolLabel(r)}`),
    datasets:[{label:'UppnÃ¥tt alla Ã¤mnen %', data:knowledgeChartRows.map(r => r.andel_uppnatt_alla_amnen), backgroundColor:'#347f6a'}]
  ...
  $('vocationalRows').innerHTML = vocationalRows.map(r => `<tr><td><strong>${esc(schoolLabel(r))}</strong></td><td>${esc(r.kon || 'Alla')}</td><td>${esc(r.elevgrupp || 'Alla')}</td><td>${esc(r.antal_elever ?? '-')}</td><td>${fmt(r.andel_behoriga_yrkesprogram, ' %')}</td></tr>`).join('') || '<tr><td colspan="5" class="muted">Ingen yrkesbehÃ¶righetsdata fÃ¶r Ã¥k 9 matchar urvalet.</td></tr>';
  ...
  makeChart('vocationalChart','bar',{
    ...
    datasets:[{label:'YrkesbehÃ¶righet Ã¥k 9 %', data:vocationalChartRows.map(r => r.andel_behoriga_yrkesprogram), backgroundColor:'#2f6f9f'}]
  },{scales:{y:{beginAtZero:true,max:100}}});
}
function renderFilteredLocal(){
  ...
  makeChart('overviewChart','bar',{
    labels: chartRows.map(r => `Ã…k ${r.arskurs} ${schoolLabel(r)}`),
    datasets:[
      {label:'MeritvÃ¤rde 17', data:chartRows.map(r => r.genomsnittligt_meritvarde_17), backgroundColor:'#2f6f9f'},
      {label:'YrkesbehÃ¶righet %', data:chartRows.map(r => r.andel_behoriga_yrkesprogram), backgroundColor:'#347f6a'}
    ]
  ...
  }).join('') || '<tr><td colspan="7" class="muted">Ingen kÃ¶nsuppdelad meritdata matchar urvalet.</td></tr>';
  const genderChartRows = genderRows.slice(0, 12);
  makeChart('genderChart','bar',{
    labels: genderChartRows.map(r => `Ã…k ${r.arskurs} ${r.school}`),
    datasets:[
...
function renderLocalData(local){
  ...
  state.tables = [];
  state.metrics = [
    {key:'localScb', label:local.isDemo ? 'Demodata' : 'Lokal SCB-import', status:'ok', reason:`${local.manifest.arskurser?.map(a => `Ã¥k ${a.arskurs}: ${a.rows}`).join(', ') || 'data finns'}`}
  ];
  ...
  $('availabilityRows').innerHTML = [
    [local.isDemo ? 'Demodata' : local.sourceKind === 'processed' ? 'Bearbetad publiceringsdata' : 'Ã…rets SCB-betyg', 'OK', `LÃ¤st frÃ¥n ${local.base}`],
    ['SV/SVA-jÃ¤mfÃ¶relse', hasSvSvaRows ? 'OK' : 'Ej hittad', 'BerÃ¤knas frÃ¥n Sv/Sva-kolumnerna.'],
    ['NP andel godkÃ¤nda', (local.npPass || []).length ? 'OK' : 'Ej hittad', 'BerÃ¤knas frÃ¥n lokal NP-import per kommun och skolenhet.'],
    ['Relation betyg och NP', (local.npRelation || []).length ? 'OK' : 'Ej hittad', 'KrÃ¤ver bÃ¥de betygsfil och NP-fil med matchande elev/skolenhet.'],
    ['PxWeb/Kolada fallback', 'Ej anvÃ¤nd', 'Lokal SCB-import hittades och anvÃ¤nds som primÃ¤r kÃ¤lla.']
  ].map(r => `<tr><td><strong>${esc(r[0])}</strong></td><td class="${r[1] === 'OK' ? 'ok' : 'warn'}">${esc(r[1])}</td><td>${esc(r[2])}</td></tr>`).join('');
  $('tableRows').innerHTML = (local.manifest.files || []).map(f => `<tr><td><strong>${esc(f.file)}</strong></td><td class="ok">lÃ¤st</td><td>${esc(f.rows)} rader</td></tr>`).join('') || '<tr><td colspan="3" class="muted">Manifestet innehÃ¥ller inga filrader.</td></tr>';
  log('Lokal SCB-import anvÃ¤nds', local.manifest);
  setStatus(local.isDemo ? 'warn' : 'ok', local.isDemo ? 'Demodata laddad.' : 'Lokal SCB-import laddad.', `${local.isDemo ? 'Visar anonym testdata' : 'Visar anonymiserad statistik'} frÃ¥n ${local.base}.`);
}
